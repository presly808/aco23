...
public class Java8Utils {
    ...
    public static List<User> topBySalaryWithLimit(List<User> list, int limit) {
        return list.stream().sorted((u1, u2) -> Double.compare(u2.money, u1.money)).limit(limit).collect(Collectors.toList());
    }
    ...
    public static Map<Department, Double> groupByDepartmentWithSumOfSalaries(List<User> list, int limit) {
        return list.stream()
                .collect(Collectors.groupingBy(user -> user.department, Collectors.summingDouble(user -> user.money)))
                .entrySet().stream().limit(limit).collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
    }
    ...
    public static Double amountOfAllSalariesUsingReduce(List<User> list) {
        return list.stream().mapToDouble(user -> user.money).reduce((money1, money2) -> money1 + money2).getAsDouble();
    }

    public static Double maxSalariesViaCollector(List<User> list) {
        return list.stream().max(Comparator.comparingDouble(u -> u.money)).get().money;
    }

    public static Map<String, List<User>> groupByName(List<User> list) {
        return list.stream().collect(Collectors.groupingBy(user -> user.name));
    }
